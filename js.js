    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case 'up':
                ...
                if (firstPoint.y - 1 == -1) {
                    return { x: firstPoint.x, y: firstPoint.y + this.config.getRowsCount() - 1 }
                }
                return { x: firstPoint.x, y: firstPoint.y - 1 };
            case 'right':
                // 3. *Убрать границы поля. Т.е. при пересечении границы поля змейка появляется с противоположной стороны.
                if (firstPoint.x + 1 == this.config.getColsCount()) {
                    return { x: firstPoint.x - this.config.getColsCount() + 1, y: firstPoint.y }
                }
...
const game = {
    ...
    canMakeStep() {
        ...
        return !this.snake.isOnPoint(nextHeadPoint) &&
            ...
            nextHeadPoint.y >= 0 &&
            // 2. Генерировать временные препятствия на поле.
            !(nextHeadPoint.x == this.barrier.x &&
                nextHeadPoint.y == this.barrier.y)
    },
